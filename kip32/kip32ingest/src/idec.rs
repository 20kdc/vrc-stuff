...
/// Metadata
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Kip32SetRegisterSource {
    /// Came directly from an AUIPC instruction.
    AUIPC,
    /// Came directly from a LUI instruction.
    LUI,
    /// Generated by the folding logic in this file.
    Folding,
    /// Generated by instruction fusion.
    Fusion,
    /// Other
    Other,
}
...
/// Metadata
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Kip32NOPSource {
    FENCE,
    WriteZero,
    /// Fusion (jump squashing)
    Fusion,
    /// Other
    Other,
}
...
pub enum Sci32Instr {
    ...
    SetRegister {
        ...
        /// Metadata only, can and should be safely ignored.
        meta_source: Kip32SetRegisterSource,
    },
    JumpAndLink {
        rd: Kip32Reg,
        rd_value: u32,
...
impl Sci32Instr {
    ...
    pub fn decode_no_opt(pc: u32, ci: u32) -> Sci32Instr {
        ...
        if opcode == 0x17 {
            ...
            Sci32Instr::SetRegister {
                ...
                meta_source: Kip32SetRegisterSource::AUIPC,
            }
        } else if opcode == 0x37 {
            // LUI
            Sci32Instr::SetRegister {
                ...
                meta_source: Kip32SetRegisterSource::LUI,
            }
        } else if opcode == 0x6F {
            // JAL
    ...
    pub fn optimize_alu(op: Sci32ALUOp) -> Sci32Instr {
        ...
        if op.rd == Kip32Reg::Zero {
            return Sci32Instr::NOP(Kip32NOPSource::WriteZero);
        }
            ...
            if let Sci32ALUSource::Immediate(i2) = op.s2 {
                ...
                if let Some(value) = op.kind.simulate(i1, i2) {
                    ...
                    return Sci32Instr::SetRegister {
                        rd: op.rd,
                        value: Sci32ALUSource::Immediate(value),
                        meta_source: Kip32SetRegisterSource::Folding,
                    };
                }
    ...
    /// Optimizes this instruction to 'break down' NOPs.
    pub fn optimize(&self) -> Self {
        match self {
            Self::SetRegister {
                ...
                value: _,
                meta_source: _,
            } => Self::NOP(Kip32NOPSource::WriteZero),
            Self::ALU(op) => Self::optimize_alu(*op),
            _ => *self,
        }
    }

    /// Decodes with constant folding/etc.
    pub fn decode(pc: u32, ci: u32) -> Sci32Instr {
        Self::decode_no_opt(pc, ci).optimize()
    }
}
