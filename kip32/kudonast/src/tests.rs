...
fn crosscheck() {
    ...
    let true_entries = kudonodin::OdinEntry::read_all_from_slice(true_odin_binary).expect("decode must succeed");
    let true_file = kudonodin::OdinASTFile::from_entries(true_entries);

    let pcfg = ron::ser::PrettyConfig::new().indentor("\t");

    let reference_ron = ron::ser::to_string_pretty(&true_file, pcfg.clone()).expect("should translate properly");
